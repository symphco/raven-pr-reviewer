...
const ignoreKeyword = '@coderaven: ignore'
...
`
  ...
  const doSummary = async (
    ...
    try {
      ...
      if (summarizeResp === '') {
        info('summarize: nothing obtained from coderaven')
        summariesFailed.push(`${filename} (nothing obtained from coderaven)`)
        return null
      } else {
      ...
      warning(`summarize: error from coderaven: ${e as string}`)
      summariesFailed.push(
        `${filename} (error from coderaven: ${e as string})})`
      )
      return null
    }
  }
...
`
      ...
      if (summarizeResp === '') {
        warning('summarize: nothing obtained from coderaven')
      } else {
  ...
  if (options.disableReleaseNotes === false) {
    ...
    if (releaseNotesResponse === '') {
      info('release notes: nothing obtained from coderaven')
    } else {
      let message = '### Summary by Raven (AI)\n\n'
...
### Chat with 🤖 Raven AI (\`@coderaven\`)
- Reply on review comments left by this bot to ask follow-up questions. A review comment is a comment on a diff or a file.
- Invite the bot into a review comment chain by tagging \`@coderaven\` in a reply.
...
- Type \`@coderaven: ignore\` anywhere in the PR description to ignore further reviews from the bot.
...
`
      ...
      try {
        ...
        if (response === '') {
          info('review: nothing obtained from coderaven')
